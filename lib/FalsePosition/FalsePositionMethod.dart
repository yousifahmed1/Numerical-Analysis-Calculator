...
class _FalsePageState extends State<FalsePage> {
  ...
  double False_P_Method(String equation, double start, double end, double tolerance,List<Map<String, dynamic>> iterations) {
    double xl = start;
    double xu = end;
    double xrOld = 0;
    double error = 0;
    ...
    if (fxl * fxu < 0) {
      do {
        ...
        if (iter == 0) {
          print(
              "iteration: $iter | xl= ${xl.toStringAsFixed(3)} | f(xl)= ${fxl.toStringAsFixed(3)} | xu= ${xu.toStringAsFixed(3)} | f(xu)= ${fxu.toStringAsFixed(3)} | xr= ${xr.toStringAsFixed(3)} | f(xr)= ${fxr.toStringAsFixed(3)}");
        } else {
          print(
  ...
  void applyBisection() {
    ...
    double result = False_P_Method(equation, start, end, tolerance, iterations);

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => FalseTablePage(iterations: iterations,lastXR:result),
      ),
    );
  }
